...
PYTEST_CI_ARGS ?= --color=yes --cov=. --cov-report=html:coverage/ --cov-append --durations=100 --maxprocesses=4 -n auto
...
# Disabling --leak-check full as for whatever reason the cudaMallocs are always found to be leaky.
# Need to track down what is going on here.
COMPUTE_SANITIZER_CMD := compute-sanitizer --launch-timeout 120 --padding 2048 --tool memcheck --error-exitcode 1
...
memcheck_tests:
	$(COMPUTE_SANITIZER_CMD) pytest -m '$(MEMCHECK_MARKER) and not $(NIGHTLY_MARKER)' $(PYTEST_CI_ARGS) --maxprocesses=1
...
nightly_memcheck_tests:
	pytest -m '$(NIGHTLY_MARKER) and $(MEMCHECK_MARKER)' $(PYTEST_CI_ARGS)
