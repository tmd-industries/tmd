...
from tmd.constants import DEFAULT_TEMP, ONE_4PI_EPS0
from tmd.fe import rbfe
from tmd.fe.free_energy import HostConfig
from tmd.fe.single_topology import SingleTopology
from tmd.fe.utils import set_mol_name
from tmd.ff import Forcefield
from tmd.lib import custom_ops
from tmd.md.builders import build_protein_system
from tmd.potentials import Nonbonded
from tmd.potentials.potential import GpuImplWrapper_f32
from tmd.potentials.types import PotentialFxn
...
def fixed_overflowed(a):
    """Refer to tmd/cpp/src/kernels/k_fixed_point.cuh::FLOAT_TO_FIXED_ENERGY for documentation on how we handle energies and overflows"""
...
def prepare_single_topology_initial_state(st: SingleTopology, host_config: Optional[HostConfig], lamb: float = 0.1):
    ...
    if host_config is not None:
        host = rbfe.setup_optimized_host(host_config, [st.mol_a, st.mol_b], st.ff)
    ...
    initial_state = rbfe.setup_initial_states(
        st, host, temperature, [lamb], seed=2022, verify_constraints=True, min_cutoff=0.7 if is_complex_leg else None
    )[0]
...
class GradientTest(unittest.TestCase):
    ...
    def compare_forces(
        ...
        params: NDArray,
        box: NDArray,
        ref_potential: PotentialFxn | GpuImplWrapper_f32,
        test_potential: GpuImplWrapper_f32,
        rtol: float,
        atol: float = 1e-8,
    ...
    def assert_differentiable_interface_consistency(
        self, x: NDArray, params: NDArray, box: NDArray, gpu_impl: GpuImplWrapper_f32
    ):
...
def load_split_forcefields() -> SplitForcefield:
    ...
    ff_path = "smirnoff_2_0_0_sc.py"

    ff_ref = Forcefield.load_from_file(ff_path)

    ff_intra = Forcefield.load_from_file(ff_path)
    ...
    ff_env = Forcefield.load_from_file(ff_path)
    ...
    ff_scaled = Forcefield.load_from_file(ff_path)
...
def hash_file(path: Path | str, chunk_size: int = 2048) -> str:
    assert Path(path).is_file(), f"{path!s} doesn't exist"
    m = hashlib.sha256()
