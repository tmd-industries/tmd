...
from tmd.fe.free_energy import InitialState, MDParams, get_water_sampler_params, sample
from tmd.fe.model_utils import apply_hmr, image_frame
...
from tmd.md.exchange.exchange_mover import get_water_idxs, translate_coordinates
...
def test_bd_exchange_get_set_params(precision):
    ...
    rng = np.random.default_rng(2023)
    proposals_per_move = 1
    params = rng.random(size=(N, 4), dtype=precision)
    ...
    with pytest.raises(RuntimeError, match="number of params don't match"):
        exchange_move.set_params(rng.random(size=(N, 3), dtype=precision))
...
def test_pair_of_waters_in_box(proposals_per_move, total_num_proposals, batch_size, precision, rtol, atol, seed):
    ...
    host_conf, _, all_group_idxs, nb = get_water_system_and_all_group_idxs(4.0)
    host_conf = host_conf.astype(precision)
...
@pytest.mark.parametrize("precision,rtol,atol", [(np.float32, 1e-4, 2e-3)])
@pytest.mark.parametrize("seed", [2023])
def test_moves_in_a_water_box(
    ...
    host_config = builders.build_water_system(box_size, ff.water_ff, box_margin=0.1)
    nb = host_config.host_system.nonbonded_all_pairs
    conf = host_config.conf.astype(precision)
    box = host_config.box.astype(precision)
    bond_list = get_bond_list(host_config.host_system.bond.potential)
...
def test_compute_incremental_log_weights(batch_size, samples, box_size, precision, rtol, atol, seed):
    ...
    for _ in range(samples):
        ...
        quaternions = rng.normal(loc=0.0, scale=1.0, size=(batch_size, 4)).astype(precision)
...
def hif2a_complex():
    ...
    with path_to_internal_file("tmd.testsystems.fep_benchmark.hif2a", "5tbm_prepared.pdb") as path_to_pdb:
        host_config = builders.build_protein_system(str(path_to_pdb), ff.protein_ff, ff.water_ff, box_margin=0.1)
    bond_list = get_bond_list(host_config.host_system.bond.potential)
    conf = host_config.conf.astype(np.float32)
    box = host_config.box.astype(np.float32)
    all_group_idxs = get_group_indices(bond_list, conf.shape[0])
    ...
    return replace(host_config, conf=conf, box=box)
