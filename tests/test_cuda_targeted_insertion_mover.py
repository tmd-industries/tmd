...
from tmd.fe.model_utils import image_frame
...
from tmd.md.exchange.exchange_mover import (
...
def compute_ref_raw_log_prob(
    ...
    """Modified from tmd.md.exchange.exchange_mover.TIBDExchangeMove.swap_vi_into_vj
...
def test_tibd_exchange_validation(precision):
    ...
    params = rng.random(size=(N, 4)).astype(precision)
...
def test_tibd_exchange_get_set_params(precision):
    ...
    with pytest.raises(RuntimeError, match="number of params don't match"):
        exchange_move.set_params(rng.random(size=(N, 3)).astype(precision))


@pytest.fixture(scope="module")
def brd4_rbfe_state() -> InitialState:
    with path_to_internal_file("tmd.testsystems.water_exchange", "brd4_pair.sdf") as ligand_path:
    ...
    # the binding pocket
    with path_to_internal_file("tmd.testsystems.water_exchange", "brd4_no_water.pdb") as pdb_path:
        host_config = builders.build_protein_system(
            str(pdb_path), ff.protein_ff, ff.water_ff, mols=[mol_a, mol_b], box_margin=0.1
        )
...
@pytest.mark.parametrize("radius", [0.46])
...
def test_targeted_insertion_buckyball_edge_cases(
    ...
    host_config = builders.build_protein_system(str(host_pdb), ff.protein_ff, ff.water_ff, mols=[mol], box_margin=0.1)

    box = host_config.box
    num_water_atoms = host_config.num_water_atoms
    assert num_water_atoms == len(host_config.conf)
    ...
    conf = afe.prepare_combined_coords(host_config.conf).astype(precision)

    bps = [pot.bind(p.astype(precision)) for pot, p in zip(potentials, params)]
...
def test_targeted_insertion_brd4_rbfe_with_context(
    ...
    conf = initial_state.x0.astype(precision)
    box = initial_state.box0.astype(precision)

    bps = initial_state.potentials
    water_params = get_water_sampler_params(initial_state).astype(precision)
    nb = get_bound_potential_by_type(bps, Nonbonded)
    bond_pot = get_bound_potential_by_type(bps, HarmonicBond).potential
...
def test_targeted_moves_in_bulk_water(
    ...
    conf = conf.astype(precision)
    ref_box = ref_box.astype(precision)
...
@pytest.mark.parametrize("precision,rtol,atol", [(np.float64, 5e-6, 6e-6), (np.float32, 1e-4, 2e-3)])
@pytest.mark.parametrize("seed", [2023])
