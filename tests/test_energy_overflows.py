...
from tmd.potentials import Nonbonded, NonbondedExclusions, NonbondedInteractionGroup
...
def test_energy_overflow_cancelled_by_exclusions(precision, rtol, atol):
    ...
    atom_idxs = np.arange(potential.num_atoms, dtype=np.int32)
    ...
    def compute_potential_energy(pot):
        ...
        # If there are no overflows, the fixed energy value will match up to floating point precision
        if not fixed_overflowed(fixed_energy):
            assert precision(fixed_to_float(fixed_energy)) == precision(selective_energy)

        return bound_energy

    reference_energy = potential(x0, params, box)
    test_energy = compute_potential_energy(potential)
    np.testing.assert_allclose(precision(reference_energy), precision(test_energy), rtol=rtol, atol=atol)
...
def test_energy_overflows_with_summation_of_energies(precision):
    ...
    box = box.astype(precision)
    nb_params = nb_params.astype(precision)
