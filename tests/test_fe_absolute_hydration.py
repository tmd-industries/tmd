...
from tmd.ff.handlers.nonbonded import AM1CCCHandler, PrecomputedChargeHandler
...
def test_fit_solvent_absolute_hydration_bcc_params(forcefield):
    seed = 2022
    n_frames = 2000
    n_eq_steps = 10000
    n_windows = 8
    ...
    def loss_fxn(bcc_params):
        ...
        # Total difference in the charge sum should stay similar. Probably a smarter way to maintain this
        difference_in_bcc = jnp.sum(jnp.abs(ref_bcc_params - bcc_params)) ** 2

        return delta_dg_term * 100.0 + difference_in_bcc * 100.0
