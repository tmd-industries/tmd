...
from tmd.md.barostat.utils import get_group_indices
from tmd.potentials import bonded, nonbonded
...
def test_get_and_set_mol_coords():
    ...
    x1 = x0 + np.random.randn(*x0.shape).astype(np.float32)
...
@pytest.mark.parametrize(
    "smi, macrocycle",
    [
        # CHEMBL1440 - Tetracycline fused tetracycle that shouldn't be treated as a macrocycle.
        ("CN(C)[C@@H]1C(O)=C(C(N)=O)C(=O)[C@@]2(O)C(O)=C3C(=O)c4c(O)cccc4[C@@](C)(O)[C@H]3C[C@@H]12", False),
        # CHEMBL3918408 - Non-macrocycle
        ("COc1ccc(-c2nc(-c3ccc(CO)o3)n3ccccc23)cc1", False),
        # CHEMBL2058675 - Macrocycle
        ("CC[C@H](C)[C@@H]1NC(=O)c2csc(n2)[C@H]([C@@H](C)CC)NC(=O)c2csc(n2)CNC(=O)c2nc1oc2C", True),
    ],
)
def test_is_macrocycle(smi, macrocycle):
    mol = Chem.AddHs(Chem.MolFromSmiles(smi))
    assert utils.is_macrocycle(mol) == macrocycle


@pytest.mark.parametrize("n_confs", [1, 5, 10])
...
def test_generate_conformations(smi, seed, n_confs):
    mol = Chem.AddHs(Chem.MolFromSmiles(smi))
