...
from tmd.ff.handlers.openmm_deserializer import deserialize_system
...
def test_protein_builder_magnesium_ion():
    pdb_str = """HETATM    1 MG    MG A   1      -9.441 -31.331  25.526  1.00 28.75          MG\n"""
    with NamedTemporaryFile(suffix=".pdb") as temp:
        with open(temp.name, "w") as ofs:
            ofs.write(pdb_str)
        builders.build_protein_system(temp.name, constants.DEFAULT_PROTEIN_FF, constants.DEFAULT_WATER_FF)
...
def test_openmm_deserialize_system_handles_duplicate_bonded_forces():
    ...
    Expectation is that these forces get correctly converted to TMD bound potentials. Test only exercises
