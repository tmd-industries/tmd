...
from tmd.potentials.jax_utils import get_all_pairs_indices, pairs_from_interaction_groups, pairwise_distances
from tmd.potentials.nonbonded import (
...
@pytest.mark.parametrize("test_full_path", [False, True])
def test_precomputation(seed, test_full_path):
    """Assert that nonbonded interaction groups using precomputation agree with reference nonbonded_on_specific_pairs"""
    ff = Forcefield.load_default()
    host_config = builders.build_water_system(3.0, ff.water_ff)
    nb = host_config.host_system.nonbonded_all_pairs
    conf = host_config.conf.astype(np.float64)
    box = host_config.box.astype(np.float64)
    ...
    for i in range(5):
        ...
        np.testing.assert_allclose(actual, expected, err_msg=f"Iteration {i}")
        ...
        np.testing.assert_allclose(v_ref, v_test, err_msg=f"Iteration {i}")
        np.testing.assert_allclose(gs_ref, gs_test, err_msg=f"Iteration {i}")
