...
def test_infer_node_vals_and_errs_networkx_invariant_wrt_relabeling_nodes(nx_graph_with_reference_mle_instance):
    ...
    for n, (ref_dg, ref_dg_err) in enumerate(zip(ref_dgs, ref_dg_errs)):
        np.testing.assert_allclose(g_res.nodes[idx_to_label[n]][node_val_prop], ref_dg, rtol=1e-5, atol=1e-7)
        # TODO: errors are noisy; unclear how to test consistency
...
def compare_inferred_and_ref_dgs(inferred_dgs: dict, node_vals, mse_thresh=2e-5):
    """assert inferred_dgs agree with node_vals (ignoring an additive offset)"""
    assert len(node_vals) >= len(inferred_dgs)
