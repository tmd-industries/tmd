...
from tmd import parallel
from tmd.parallel import client
from tmd.parallel.utils import batch_list
...
@pytest.mark.parametrize("pool", [client.CUDAPoolClient(4), client.CUDAMPSPoolClient(2, 2)])
def test_cuda_pool_submit(pool):
    ...
    expected_ids = [str(i) for i in range(operations)]
    assert test_ids == expected_ids
    assert test_names == expected_ids

    if isinstance(pool, client.CUDAMPSPoolClient):
        expected = [str(((i % pool.num_gpus) * i) % pool.workers_per_gpu) for i in range(operations)]
    else:
        expected = [str(i % pool.max_workers) for i in range(operations)]

    np.testing.assert_array_equal(test_res, expected)


@pytest.mark.parametrize("pool", [client.CUDAPoolClient(2), client.CUDAMPSPoolClient(2, 2)])
def test_cuda_pool_submit_kwargs(pool):
    arr = np.linspace(0, 1.0, 5)

    futures = []
    for x in arr:
        fut = pool.submit(sum, x, key=x)
        futures.append(fut)
...
@pytest.mark.parametrize("klass", [client.CUDAPoolClient, client.CUDAMPSPoolClient])
def test_cuda_pool_single_worker(klass):
