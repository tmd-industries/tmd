...
def execute_bound_impl(bp):
    ...
    bp.execute(coords.astype(np.float32), box.astype(np.float32))
...
def test_bound_potential_get_potential(harmonic_bond):
    ...
    bound_impl = custom_ops.BoundPotential_f32(unbound_impl, harmonic_bond.params)
...
def test_bound_potential_empty_params():
    ...
    u_test.bound_impl.set_params(np.empty((0, 2), dtype=np.float32))
    assert u_test(x, box) == 0.0


def test_bound_potential_set_params(harmonic_bond):
    x, box = np.ones((3, 3), dtype=np.float32), np.eye(3, dtype=np.float32)
...
def verify_potential_validation(potential):
    with pytest.raises(RuntimeError, match="coords dimensions must be 2"):
        potential(np.zeros(1, dtype=np.float32), np.ones((3, 3), dtype=np.float32))

    with pytest.raises(RuntimeError, match="coords must have a shape that is 3 dimensional"):
        potential(np.zeros((1, 4), dtype=np.float32), np.ones((3, 3), dtype=np.float32))

    with pytest.raises(RuntimeError, match="box must be 3x3"):
        potential(np.zeros((1, 3), dtype=np.float32), np.ones(3, dtype=np.float32))

    with pytest.raises(RuntimeError, match="box must be 3x3"):
        potential(np.zeros((1, 3), dtype=np.float32), np.ones((2, 2), dtype=np.float32))

    with pytest.raises(RuntimeError, match="box must be ortholinear"):
        potential(np.zeros((1, 3), dtype=np.float32), np.ones((3, 3), dtype=np.float32))

    with pytest.raises(RuntimeError, match="box must have positive values along diagonal"):
        potential(np.zeros((1, 3), dtype=np.float32), np.zeros((3, 3), dtype=np.float32))
...
def test_unbound_potential_execute_validation(harmonic_bond):
    ...
    def func(coords, box):
        return unbound_impl.execute(coords, harmonic_bond.params.astype(np.float32), box)
...
def test_summed_potential_get_potentials(harmonic_bond):
    ...
    summed_impl = custom_ops.SummedPotential_f32(impls, params_sizes)
...
def test_summed_potential_invalid_parameters_size(harmonic_bond):
    ...
    assert (
        f"SummedPotential<RealType>::execute_device(): expected {harmonic_bond.params.size} parameters, got 0" in str(e)
    )
    ...
    assert (
        f"SummedPotential<RealType>::execute_device(): expected {harmonic_bond.params.size} parameters, got {harmonic_bond.params.size + 1}"
        in str(e)
    )
...
def test_summed_potential(parallel, num_potentials, harmonic_bond_test_system):
    ...
    for rtol, precision in [(1e-6, np.float32), (1e-10, np.float64)]:
        x = coords.astype(precision)
        p = flat_params.astype(precision)
        b = box.astype(precision)
        GradientTest().compare_forces(x, p, b, potential, potential.to_gpu(precision), rtol)
...
def test_execute_batch_sparse_validation(harmonic_bond: BoundPotential[HarmonicBond], precision):
    unbound_impl = harmonic_bond.potential.to_gpu(precision).unbound_impl

    # Should verify that number of boxes and coords match
    with pytest.raises(RuntimeError) as e:
        _ = unbound_impl.execute_batch_sparse(
            np.zeros((5, 4, 3), dtype=precision),
            np.zeros((4, 3, 2), dtype=precision),
            np.zeros((6, 3, 3), dtype=precision),  # inconsistent length
    ...
    with pytest.raises(RuntimeError) as e:
        _ = unbound_impl.execute_batch_sparse(
            np.zeros((5, 4, 3), dtype=precision),
            np.zeros((4, 3, 2), dtype=precision),
            np.zeros((5, 3), dtype=precision),  # missing dimension
            np.zeros(3).astype(np.uint32),
            np.zeros(3).astype(np.uint32),
            True,
            True,
    ...
    with pytest.raises(RuntimeError) as e:
        _ = unbound_impl.execute_batch_sparse(
            np.zeros((5, 4), dtype=precision),  # missing dimension
            np.zeros((4, 3, 2), dtype=precision),
            np.zeros((5, 3, 3), dtype=precision),
            np.zeros(3).astype(np.uint32),
            np.zeros(3).astype(np.uint32),
            True,
            True,
    ...
    with pytest.raises(RuntimeError) as e:
        _ = unbound_impl.execute_batch_sparse(
            np.zeros((5, 4, 3), dtype=precision),
            np.zeros(4, dtype=precision),  # 1-d
            np.zeros((5, 3, 3), dtype=precision),
            np.zeros(3).astype(np.uint32),
            np.zeros(3).astype(np.uint32),
            True,
