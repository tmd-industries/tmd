...
def test_reversibility_with_custom_ops_potentials():
    ...
    host = None  # vacuum
    initial_states = setup_initial_states(rfe, host, temperature, [lamb], seed, verify_constraints=True)
    ...
    box = 100 * np.eye(3, dtype=np.float32)

    def force(coords):
        du_dxs = np.array([bp.execute(coords.astype(np.float32), box)[0] for bp in bound_potentials])
        return -np.sum(du_dxs, 0)

    dt = 1.5e-3
    intg = VelocityVerletIntegrator(force, masses, dt)

    x0 = np.array(coords).astype(np.float32)
