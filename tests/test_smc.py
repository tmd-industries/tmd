...
from tmd.testsystems.gaussian1d import make_gaussian_testsystem
...
def test_conditional_effective_sample_size():
    ...
    for _ in range(100):
        ...
        n_particles = 10000
        u_fxn, _, sample, reduced_free_energy, _ = make_gaussian_testsystem()
...
@pytest.mark.parametrize("lamb_start, lamb_end", [(0.0, 1.0), (0.5, 0.85), (1.0, 0.0)])
def test_sequential_monte_carlo(
    max_iteration_test: bool,
    cess_factor: Optional[float],
    test_adaptive_smc: bool,
    resampling_fxn: Resampler,
    lamb_start: float,
    lamb_end: float,
):
    ...
    * endstate samples have expected mean and stddev
    """
    np.random.seed(2022)
    assert 0.0 <= lamb_start <= 1.0
    assert 0.0 <= lamb_end <= 1.0
    assert lamb_start != lamb_end
    ...
    n_particles = 10000
    u_fxn, _, sample, reduced_free_energy, annealed_gaussian_def = make_gaussian_testsystem()

    # prepare inputs for SMC
    samples = sample(lamb_start, params, n_particles).flatten()
    ...
    if test_adaptive_smc:
        ...
        cess_target = len(samples) * cess_factor  # arbitrary value results in ~5 windows
        find_next_lambda = functools.partial(
            adaptive_find_next_lambda, log_prob=log_prob, cess_target=cess_target, final_lambda=lamb_end
        )
        if cess_target > len(samples):
            with pytest.raises(AssertionError, match="too large"):
                result_dict = sequential_monte_carlo(
                    ...
                    resampling_fxn,
                    find_next_lambda,
                    initial_lamb=lamb_start,
                    target_lamb=lamb_end,
                )
            return
        ...
        # apply SMC
        n_windows = 100
        lambdas = np.linspace(lamb_start, lamb_end, n_windows)
    ...
    assert lambdas[0] == lamb_start
    assert lambdas[-1] == lamb_end
    ...
    np.testing.assert_allclose(running_estimates, ref_delta_fs, atol=0.04)
    ...
    assert int(mse_vs_constant / mse_vs_smc_estimates) > 1000
    ...
    expected_start_mean, expected_start_std = annealed_gaussian_def(lamb_start, params)
    expected_end_mean, expected_end_std = annealed_gaussian_def(lamb_end, params)

    np.testing.assert_almost_equal(mean_0, expected_start_mean, decimal=1)
    np.testing.assert_almost_equal(np.log(stddev_0), np.log(expected_start_std), decimal=1)

    np.testing.assert_almost_equal(mean_1, expected_end_mean, decimal=1)
    np.testing.assert_almost_equal(np.log(stddev_1), np.log(expected_end_std), decimal=1)
