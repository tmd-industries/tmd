...
from tmd.fe.topology import _SCALE_14_LJ, _SCALE_14_Q, BaseTopology, DualTopology, MultiTopology
...
@pytest.mark.skip(reason="Requires AM1 for the Residue charges")
@no_type_check
...
def host_guest_nonbonded_impl(ctor, precision, rtol, atol, use_tiny_mol):
    ...
    def compute_new_grad_u(ff: Forcefield, precision, x0, box, lamb, num_water_atoms, host_system, omm_topology):
        ...
        bt = Topology(ff)
        hgt = topology.HostGuestTopology(host_system.get_U_fns(), bt, num_water_atoms, ff, omm_topology)
        params, nb_allpairs = hgt.parameterize_nonbonded(
            ff.q_handle.params,
            ff.q_handle_intra.params,
            ff.lj_handle.params,
            ff.lj_handle_intra.params,
        ...
        nb = nb_allpairs.bind(params)
        pairlist_params, nb_pairlist = hgt.parameterize_nonbonded_pairlist(
            ff.q_handle.params,
            ff.q_handle_intra.params,
            ff.lj_handle.params,
            ff.lj_handle_intra.params,
        )
        pairlist = nb_pairlist.bind(pairlist_params)

        summed_pot = potentials.make_summed_potential([nb, pairlist])
        u_impl = summed_pot.to_gpu(precision=precision).bound_impl
        return u_impl.execute(x0, box)
    ...
    elif ctor == MultiTopology:
        mols = [Chem.Mol(mol) for mol in mols_by_name.values()]
        new_center = np.mean(get_romol_conf(mols[0]), axis=0)
        for mol in mols:
            coords = get_romol_conf(mol)
            center = np.mean(coords, axis=0)
            set_romol_conf(mol, coords - center + new_center)
        Topology = partial(ctor, mols)
        ligand_conf = np.concatenate([get_romol_conf(m) for m in mols_by_name.values()])
    else:
