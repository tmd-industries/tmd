...
from tmd.integrator import VelocityVerletIntegrator as ReferenceVelocityVerlet
from tmd.lib import VelocityVerletIntegrator, custom_ops
from tmd.potentials import SummedPotential
from tmd.testsystems.relative import get_relative_hif2a_in_vacuum


# (YTZ): When refactoring the context to the 32bit implementation, it turned out that
# many of these tests need to use the 64bit implementation in order to pass, otherwise
# the tolerances in assert_reversible need to be loosened significantly
...
def test_reversibility():
    ...
    bound_impls = [pot.to_gpu(precision=np.float64).bound_impl for pot in bound_potentials]
...
def test_matches_reference():
    ...
    bound_summed = summed_potential.bind(sp_params).to_gpu(np.float64).bound_impl
    ...
    bound_impls = [bp.to_gpu(np.float64).bound_impl for bp in bound_potentials]
    ctxt = setup_velocity_verlet(bound_impls, coords, box, dt, masses)
