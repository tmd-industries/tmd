...
from tmd._vendored.pymbar.utils import kln_to_kn, ParameterError, DataError, logsumexp
from tmd._vendored.pymbar import timeseries
...
class FES:
    ...
    def __init__(self, u_kn, N_k, verbose=False, mbar_options=None, timings=True, **kwargs):
        ...
        >>> from tmd._vendored.pymbar import testsystems
    ...
    def generate_fes(
        ...
        """
        Given an initialized MBAR object, a set of points,
        ...
        seed : int, Default = -1
            Set the randomization seed. Setting should get the
        ...
        * fes_type = 'histogram':
        ...
        >>> from tmd._vendored.pymbar import testsystems
        >>> from tmd._vendored.pymbar import FES
        ...
        for b in range(n_bootstraps + 1):  # generate bootstrap samples.
            ...
            if b == 0:  # the default
                ...
                index = 0
                for k in range(K):
                    # TODO: address issue with storing np.random in self
    ...
    def _setup_fes_histogram(self, histogram_parameters):
        """
        Does initial processing of histogram_parameters

        Parameters
        ----------
        histogram_parameters : dict()
            A options:values dictionary for parameters to create the FES using histograms
    ...
    def _setup_fes_kde(self, kde_parameters):
        """
        Does initial processing of kde_parameters

        Parameters
        ----------
        kde_parameters : dict()
            A options:values dictionary for parameters to create the FES using the kernel density approach.
            Parameters are passed on to sklearn KernelDensity.

        Returns
        -------
        None

        Internally, creates kde object and kdes list of kde objects
    ...
    def _setup_fes_spline(self, spline_parameters):
        ...
        spline_parameters : dict()
            A options:values dictionary for parameters to create the FES using the spline approach.
        ...
        # we need to initialize our function starting point
    ...
    def _get_initial_spline_points(self):
        ...
        points to create a spline from which to start the minimization.
        ...
        -------
        xinit : ndarray, float
            x-values of spline to be fit for start of minimization
        yinit : ndarray, float, shape
            y-values of spline to be fit for start of minimization
    ...
    def _calculate_information_criteria(nparameters, minus_log_likelihood, N):
        """
        Calculate and store various information criterias
        ...
        results : dictionary
            Keys are "aic" (Akaike information criteria) and "bic" (Bayesian information criteria)
    ...
    def _get_fes_kde(
        ...
        if uncertainty_method is None:
            ...
            if self.kdes is None:
                raise ParameterError(
                    f"Cannot calculate bootstrap error of bootstrap KDE's not determined"
                )
            else:
    ...
    def _get_fes_spline(
        ...
        if uncertainty_method == "bootstrap":
            if self.fes_functions is None:
                raise ParameterError(
                    "Cannot calculate via uncertainties error if bootstrapping was not performed running get_fes"
                )
            else:
    ...
    def sample_parameter_distribution(
        ...
        decorrelate : boolean, optional, default=True
        ...
        self.mc_data["g_parameters"] = g_c  # statistical efficiency of the parameters
    ...
    def get_mc_data(self):
        ...
        dict
            ...
            mc_parameters : dictionary of parameters that were run with (see definitions in sample_parameter_distrbution)
            acceptance_ratio : overall acceptance ratio of the MC chain
            nequil : the start of the "equilibrated" data set (i.e. nequil-1 is the number that were thrown out)
            g_logposterior : statistical efficiency of the log posterior
            g_parameters : statistical efficiency of the parameters
    ...
    def _MC_step(self, x_n, w_n, stepsize, xrange, spline_weights, logprior):
        ...
        xrange:
            Range the probability distribution is defined over.
        spline_weights:
...
# for k=2, it is piecewise sum of 3 quadratic terms, which is quadratic
