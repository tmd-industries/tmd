...
namespace tmd {
// The number of threads per block for the setting of the final weight of the
// moved mol is low if using the same number as in the rest of the kernels of
...
BDExchangeMove<RealType>::BDExchangeMove(
    ...
    : Mover<RealType>(interval), N_(N), mol_size_(target_mols[0].size()),
      ...
      d_before_mol_energy_buffer_(num_target_mols_),
      d_proposal_mol_energy_buffer_(num_target_mols_ * batch_size_),
      d_sample_per_atom_energy_buffer_(batch_size_ * mol_size_ * N),
      d_atom_idxs_(get_atom_indices(target_mols)),
      d_mol_offsets_(get_mol_offsets(target_mols)),
      d_log_weights_before_(num_target_mols_),
      d_log_weights_after_(batch_size_ * num_target_mols_),
      d_lse_max_before_(1), d_lse_exp_sum_before_(1),
      d_lse_max_after_(batch_size_), d_lse_exp_sum_after_(batch_size_),
      ...
      d_sampling_intermediate_(num_target_mols_ * batch_size_),
      d_translations_(translation_buffer_size),
      d_sample_segments_offsets_(batch_size_ + 1), d_noise_offset_(1),
      p_noise_offset_(1) {
  ...
  for (int i = 0; i < target_mols.size(); i++) {
    if (target_mols[i].size() != mol_size_) {
      throw std::runtime_error(
          "only support running with mols with constant size, got mixed sizes");
    }
  }
  // Clear out the logsumexp values so the log probability starts off as zero
  gpuErrchk(
      cudaMemset(d_lse_exp_sum_before_.data, 0, d_lse_exp_sum_before_.size()));
  gpuErrchk(cudaMemset(d_lse_max_before_.data, 0, d_lse_max_before_.size()));
  gpuErrchk(
      cudaMemset(d_lse_exp_sum_after_.data, 0, d_lse_exp_sum_after_.size()));
  ...
  // Initialize several different RNGs to allow for determinism between numbers
  // of steps per move
  ...
  curandErrchk(
      curandCreateGenerator(&cr_rng_translations_, CURAND_RNG_PSEUDO_DEFAULT));
  curandErrchk(
      curandSetPseudoRandomGeneratorSeed(cr_rng_translations_, seed + 1));

  curandErrchk(
      curandCreateGenerator(&cr_rng_samples_, CURAND_RNG_PSEUDO_DEFAULT));
  ...
  // constant for BDExchangeMove since the sample size is always batches of
  // num_target_mols_ weights
...
void BDExchangeMove<RealType>::compute_initial_log_weights_device(
  ...
  // Copy the same mol energies repeatedly from the before energies to the
  // proposal energies
  k_copy_batch<__int128>
      <<<dim3(ceil_divide(num_target_mols_, tpb), batch_size_, 1), tpb, 0,
         stream>>>(num_target_mols_, batch_size_,
...
void BDExchangeMove<RealType>::compute_incremental_log_weights_device(
    ...
    cudaStream_t stream) {
  const int tpb = DEFAULT_THREADS_PER_BLOCK;
  // Scale down the y block by 16 to have each block do more work. Improves
  // memory throughput Improves performance by about 10% on Tesla cards, may not
  // generalize to consumer cards
  dim3 atom_by_atom_grid(ceil_divide(N, tpb),
                         ceil_divide(mol_size_ * batch_size_, 16), 1);

  k_setup_proposals<<<ceil_divide(batch_size_, tpb), tpb, 0, stream>>>(
      num_proposals_per_move_, batch_size_, mol_size_, d_noise_offset_.data,
      d_samples_.data, d_atom_idxs_.data, d_mol_offsets_.data,
      d_target_mol_atoms_.data, d_target_mol_offsets_.data);
  gpuErrchk(cudaPeekAtLastError());

  if (scale) {
    k_rotate_and_translate_mols<RealType, true>
        <<<ceil_divide(batch_size_, tpb), tpb, 0, stream>>>(
            num_proposals_per_move_, batch_size_, d_noise_offset_.data,
            d_coords, d_box, d_samples_.data, d_target_mol_offsets_.data,
            d_quaternions, d_translations, d_intermediate_coords_.data);
    gpuErrchk(cudaPeekAtLastError());
  } else {
    k_rotate_and_translate_mols<RealType, false>
        <<<ceil_divide(batch_size_, tpb), tpb, 0, stream>>>(
            num_proposals_per_move_, batch_size_, d_noise_offset_.data,
            d_coords, d_box, d_samples_.data, d_target_mol_offsets_.data,
            d_quaternions, d_translations, d_intermediate_coords_.data);
  ...
  k_atom_by_atom_energies<RealType><<<atom_by_atom_grid, tpb, 0, stream>>>(
      ...
      d_params_.data, d_box, nb_beta_, cutoff_,
      d_sample_per_atom_energy_buffer_.data);
  gpuErrchk(cudaPeekAtLastError());

  // Subtract off the weights for the individual waters from the sampled water.
  // It modifies the sampled mol energy value, leaving it in an invalid state,
  // which is why we later call k_set_sampled_energy to set the weight of the
  // sampled mol
  k_adjust_energies<RealType, true>
      <<<dim3(ceil_divide(num_target_mols_, tpb), batch_size_, 1), tpb, 0,
         stream>>>(N, batch_size_, mol_size_, num_target_mols_,
                   d_atom_idxs_.data, d_mol_offsets_.data,
                   d_sample_per_atom_energy_buffer_.data,
                   d_proposal_mol_energy_buffer_.data);
  gpuErrchk(cudaPeekAtLastError());
  ...
  k_adjust_energies<RealType, false>
      <<<dim3(ceil_divide(num_target_mols_, tpb), batch_size_, 1), tpb, 0,
         stream>>>(N, batch_size_, mol_size_, num_target_mols_,
                   d_atom_idxs_.data, d_mol_offsets_.data,
                   d_sample_per_atom_energy_buffer_.data,
  ...
  k_set_sampled_energy_block<RealType, WEIGHT_THREADS_PER_BLOCK>
      <<<dim3(num_intermediates_per_reduce_, batch_size_, 1),
         WEIGHT_THREADS_PER_BLOCK, 0, stream>>>(
          N, batch_size_, mol_size_, num_target_mols_, d_target_mol_atoms_.data,
          d_sample_per_atom_energy_buffer_.data,
  ...
  k_set_sampled_energy_reduce<WEIGHT_THREADS_PER_BLOCK>
      <<<dim3(1, batch_size_, 1), WEIGHT_THREADS_PER_BLOCK, 0, stream>>>(
          batch_size_, num_target_mols_,
          num_intermediates_per_reduce_, // Number of intermediates per sample
                                         // in batch
          d_samples_.data,               // where to set the value
          d_intermediate_sample_weights_
              .data, // intermediate fixed point weights
          d_proposal_mol_energy_buffer_.data);
  gpuErrchk(cudaPeekAtLastError());

  k_convert_energies_to_log_weights<RealType>
      <<<ceil_divide(num_target_mols_ * batch_size_, tpb), tpb, 0, stream>>>(
          num_target_mols_ * batch_size_, beta_,
          d_proposal_mol_energy_buffer_.data, d_log_weights_after_.data);
  gpuErrchk(cudaPeekAtLastError());
}

template <typename RealType>
std::vector<std::vector<RealType>>
BDExchangeMove<RealType>::compute_incremental_log_weights_host(
    const int N,
    const RealType *h_coords, // [N, 3]
  ...
  for (unsigned int i = 0; i < this->batch_size_; i++) {
    ...
    h_output[i] = std::vector<RealType>(h_log_weights_after.begin() + start,
                                        h_log_weights_after.begin() + end);
  }
  return h_output;
}
...
template <typename RealType>
std::vector<RealType>
BDExchangeMove<RealType>::compute_initial_log_weights_host(
    const int N,
    const RealType *h_coords, // [N, 3]
    const RealType *h_box     // [3, 3]
) {
...
template <typename RealType>
std::vector<RealType> BDExchangeMove<RealType>::get_before_log_weights() {
...
template <typename RealType>
std::vector<RealType> BDExchangeMove<RealType>::get_after_log_weights() {
...
template <typename RealType>
RealType BDExchangeMove<RealType>::raw_log_probability_host() {
  ...
  RealType before_log_prob = convert_nan_to_inf(
      compute_logsumexp_final(h_log_exp_before[0], h_log_exp_before[1]));
  RealType after_log_prob = convert_nan_to_inf(compute_logsumexp_final(
      h_log_exp_after[0], h_log_exp_after[batch_size_]));

  return static_cast<RealType>(before_log_prob - after_log_prob);
}

template <typename RealType>
RealType BDExchangeMove<RealType>::log_probability_host() {
...
template <typename RealType>
std::vector<RealType> BDExchangeMove<RealType>::get_params() {
  std::vector<RealType> h_params(d_params_.length);
  d_params_.copy_to(&h_params[0]);
  return h_params;
};

template <typename RealType>
void BDExchangeMove<RealType>::set_params(const std::vector<RealType> &params) {
  cudaStream_t stream = static_cast<cudaStream_t>(0);
  DeviceBuffer<RealType> d_params(params.size());
...
template <typename RealType>
void BDExchangeMove<RealType>::set_params_device(const int size,
                                                 const RealType *d_p,
                                                 const cudaStream_t stream) {
  if (d_params_.length != size) {
    throw std::runtime_error("number of params don't match");
  }
  gpuErrchk(cudaMemcpyAsync(d_params_.data, d_p, d_params_.size(),
