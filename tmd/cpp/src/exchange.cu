...
std::array<std::vector<int>, 2> get_inner_and_outer_mols(
    const std::vector<int> &center_atoms, const std::vector<RealType> &coords,
    const std::vector<RealType> &box,
    const std::vector<std::vector<int>> &group_idxs, const RealType radius) {

  const int num_mols = group_idxs.size();
  DeviceBuffer<int> d_center_atoms(center_atoms);
  DeviceBuffer<RealType> d_coords(coords);
  ...
  k_compute_centroid_of_atoms<<<1, tpb, 0, stream>>>(
      static_cast<int>(d_center_atoms.length), d_center_atoms.data,
      d_coords.data, d_centroid.data);
  gpuErrchk(cudaPeekAtLastError());

  const RealType square_radius = radius * radius;

  k_flag_mols_inner_outer<<<ceil_divide(num_mols, tpb), tpb, 0, stream>>>(
      num_mols, d_atom_indices.data, d_mol_offsets.data, d_centroid.data,
      square_radius, d_coords.data, d_box.data, d_flags.data);
...
template std::array<std::vector<int>, 2> get_inner_and_outer_mols<float>(
    const std::vector<int> &center_atoms, const std::vector<float> &coords,
    const std::vector<float> &box,
    const std::vector<std::vector<int>> &group_idxs, const float radius);
template std::array<std::vector<int>, 2> get_inner_and_outer_mols<double>(
    const std::vector<int> &center_atoms, const std::vector<double> &coords,
    const std::vector<double> &box,
    const std::vector<std::vector<int>> &group_idxs, const double radius);
