...
template <typename RealType> __device__ struct Vector {
  ...
  __device__ RealType dot(Vector<RealType> const &other) {
  ...
  __device__ Matrix<RealType> unit_jac() const {
    ...
    return {(1 - v.x * v.x) / n, (-v.x * v.y) / n,    (-v.x * v.z) / n,
            (-v.y * v.x) / n,    (1 - v.y * v.y) / n, (-v.y * v.z) / n,
            (-v.z * v.x) / n,    (-v.z * v.y) / n,    (1 - v.z * v.z) / n};
  }
...
__device__ Vector<RealType> mul_vm(Vector<RealType> v, Matrix<RealType> m) {
...
template <typename RealType>
__device__ Vector<RealType> cross_product(Vector<RealType> a,
                                          Vector<RealType> b) {
...
template <typename RealType>
__device__ void cross_jac(Vector<RealType> a, Vector<RealType> b,
                          Matrix<RealType> &jac_a, Matrix<RealType> &jac_b) {
...
template <typename RealType>
__device__ void
pyramidal_vol_and_grad(Vector<RealType> xc, Vector<RealType> x1,
                       Vector<RealType> x2, Vector<RealType> x3, RealType &vol,
                       Vector<RealType> &xc_grad, Vector<RealType> &x1_grad,
                       Vector<RealType> &x2_grad, Vector<RealType> &x3_grad) {
...
template <typename RealType>
__device__ void
torsion_vol_and_grad(Vector<RealType> x0, Vector<RealType> x1,
                     Vector<RealType> x2, Vector<RealType> x3, RealType &vol,
                     Vector<RealType> &x0_grad, Vector<RealType> &x1_grad,
                     Vector<RealType> &x2_grad, Vector<RealType> &x3_grad) {
  ...
  auto do_dyy = mul_vm(mul_vm(do_dc0, dc0_dy), dy_dyy) +
                mul_vm(mul_vm(do_dc1, dc1_dy), dy_dyy);
