...
template <typename T>
void __global__ k_copy_batch(const int N, const int batch_size,
                             const T *__restrict__ src, T *__restrict__ dest) {
...
template void __global__ k_copy_batch<float>(const int N, const int batch_size,
                                             const float *__restrict__ src,
                                             float *__restrict__ dest);

template void __global__ k_copy_batch<double>(const int N, const int batch_size,
                                              const double *__restrict__ src,
                                              double *__restrict__ dest);

template void __global__ k_copy_batch<__int128>(
    const int N, const int batch_size, const __int128 *__restrict__ src,
    __int128 *__restrict__ dest);

template <typename RealType>
void __global__ k_convert_energies_to_log_weights(
    const int N, const RealType inv_beta, const __int128 *__restrict__ energies,
    RealType *__restrict__ log_weights) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  while (idx < N) {
    __int128 energy = energies[idx];
    log_weights[idx] = fixed_point_overflow(energy)
                           ? static_cast<RealType>(INFINITY)
                           : inv_beta * FIXED_TO_FLOAT<RealType>(energy);
    idx += gridDim.x * blockDim.x;
  }
}

template void __global__ k_convert_energies_to_log_weights<float>(
    const int N, const float inv_beta, const __int128 *__restrict__ energies,
    float *__restrict__ log_weights);
template void __global__ k_convert_energies_to_log_weights<double>(
    const int N, const double inv_beta, const __int128 *__restrict__ energies,
    double *__restrict__ log_weights);

// k_setup_proposals takes a set of sampled indices and constructs buffers
// containing the molecule offsets as well as the atom indices (refer to
// src/mol_utils.hpp for impl) and setups up the offsets and the atom indices
// for each sample. Note that the output mol offsets are constructed so that the
// start of the mol is the starting atom idx rather than the prefix sum of mol
// lengths that the mol_offsets is.
void __global__ k_setup_proposals(
    ...
    const int *__restrict__ mol_idx_per_batch, // [batch_size] The index of the
                                               // molecules to sample
  ...
  while (idx < batch_size) {
    ...
    for (int i = 0; i < num_atoms; i++) {
      output_atom_idxs[idx * num_atoms_in_each_mol + i] =
          atom_indices[mol_start + i];
    }
    idx += gridDim.x * blockDim.x;
  }
}
...
void __global__ k_store_exchange_move(
    const int batch_size, const int num_target_mols,
    ...
    const RealType *__restrict__ moved_coords,     // [num_atoms_in_each_mol, 3]
    RealType *__restrict__ dest_coords,            // [N, 3]
  ...
  // Note that this kernel does not handle multiple proposals, expects that the
  // proposals have been reduced down to a single proposal beforehand.
  int atom_idx = blockIdx.x * blockDim.x + threadIdx.x;

  const int batch_idx = accepted_batched_move[0];
  // If the selected batch idx is not less than the total batch size, no
  // proposal was accepted, we can exit immediately.
  ...
  // Increment offset by the index + 1, IE the Nth item in the batch being
  // accepted results in incrementing by N + 1
  if (atom_idx == 0) {
    if (accepted) {
  ...
  // either the after energies to the before energies or the accepted batches
  // energies to the before and the other after energies
  const int energies_copy_count = num_target_mols * batch_size;

  // If accepted, move the coords into place
  // Always copy the energies, either copying from before to after or after to
  // before
  while (atom_idx < energies_copy_count || atom_idx < num_atoms_in_mol) {
    ...
    // At the end of batch of proposals we need to update the before and after
    // energies to the correct state. In the case of not accepting any moves we
    // want to reset the after energies to be the before energies so that
    // kernels can update the after energies.
    // In the case of accepting a move the before energies need to be updated
    // with the after energies associated with
    //  the accepted batch. The accepted energies also need to be copied into
    //  the other after energy batches so that
    // the next step can accumulate energies correctly.
    // We will use `k_convert_energies_to_log_weights` to generate the correct
    // weights separately.
    if (atom_idx < energies_copy_count) {
      if (accepted) {
        // Before energies is only num_target_mols long
        if (atom_idx < num_target_mols) {
          before_energies[atom_idx] =
              after_energies[batch_idx * num_target_mols + atom_idx];
        }
        if (atom_idx !=
            batch_idx * num_target_mols + (atom_idx % num_target_mols)) {
          after_energies[atom_idx] =
              after_energies[batch_idx * num_target_mols +
                             (atom_idx % num_target_mols)];
        }
      } else {
...
void __global__ k_store_accepted_log_probability(
    const int num_energies, const int batch_size,
    const int *__restrict__ accepted_batched_move, // [1]
  ...
  const int batch_idx = accepted_batched_move[0];
  // If the selected mol is not less than the total batch size, no proposal was
  // accepted, we can exit immediately.
...
template void __global__ k_store_accepted_log_probability<float>(
    ...
    float *__restrict__ before_max, float *__restrict__ before_log_sum,
...
template void __global__ k_store_accepted_log_probability<double>(
    const int num_energies, const int batch_size,
    const int *__restrict__ accepted_batched_move,
    double *__restrict__ before_max, double *__restrict__ before_log_sum,
    const double *__restrict__ after_max,
    const double *__restrict__ after_log_sum);

template <typename RealType>
void __global__ k_compute_box_volume(const RealType *__restrict__ box,    // [3]
...
// k_adjust_energies takes a set of molecule energies and either subtracts
// (Negated=true) or adds (Negated=false) the sum of the per atom energies for
// the molecules from some initial energy. This is used to do the transposition
// trick where we subtract off the energy contribution of the moved atom
// followed by adding back in the energy of the sampled mol in the new position.
// Does NOT special case the energy of the sampled mol and instead use
// `k_set_sampled_energy_block`.
template <typename RealType, bool Negated>
void __global__ k_adjust_energies(
    const int N, const int batch_size, const int mol_size,
    const int num_energies, const int *__restrict__ mol_atoms_idxs,
  ...
  while (idx_in_batch < batch_size) {
    ...
    while (mol_idx < num_energies) {
      ...
      for (int i = idx_in_batch * mol_size; i < (idx_in_batch + 1) * mol_size;
           i++) {
        for (int j = min_atom_idx; j <= max_atom_idx; j++) {
          energy_accumulator +=
              FLOAT_TO_FIXED_ENERGY<RealType>(per_atom_energies[i * N + j]);
        }
      }

      mol_energies[idx_in_batch * num_energies + mol_idx] +=
          Negated ? -energy_accumulator : energy_accumulator;
...
void __global__ k_set_sampled_energy_block(
    ...
    const RealType *__restrict__ per_atom_energies,
    __int128 *__restrict__ intermediate_accum // [batch_size, ceil_divide(N,
                                              // THREADS_PER_BLOCK)]
) {
  ...
  while (idx_in_batch < batch_size) {
    ...
    while (atom_idx < N) {
      if (atom_idx < min_atom_idx || atom_idx > max_atom_idx) {
        for (int i = idx_in_batch * mol_size; i < (idx_in_batch + 1) * mol_size;
             i++) {
          accumulator += FLOAT_TO_FIXED_ENERGY<RealType>(
              per_atom_energies[i * N + atom_idx]);
        }
      }
      atom_idx += gridDim.x * blockDim.x;
    }
...
template void __global__ k_set_sampled_energy_block<float, 512>(
    const int N, const int batch_size, const int mol_size,
    const int num_energies, const int *__restrict__ target_atoms,
    const float *__restrict__ per_atom_energies,
...
void __global__ k_set_sampled_energy_reduce(
    const int batch_size, const int num_energies, const int num_intermediates,
    const int *__restrict__ samples, // [batch_size]
    const __int128
        *__restrict__ intermediate_accum, // [batch_size, num_intermediates]
    __int128 *__restrict__ mol_energies   // [batch_size, num_energies]
) {
  __int128 accumulator;
  ...
  // One y block per set of energies, used instead of x block to avoid nuance of
  // setting idx based only on the thread idx
  int idx_in_batch = blockIdx.y;
  ...
  while (idx_in_batch < batch_size) {
    ...
    // Each block reduces on a specific set of energies which is why just
    // threadIdx.x Sum's return value is only valid in thread 0
    __int128 aggregate = BlockReduce(temp_storage).Sum(accumulator);
    __syncthreads();
    if (threadIdx.x == 0) {
...
template void __global__ k_set_sampled_energy_reduce<512>(
    const int batch_size, const int num_intermediates, const int num_energies,
    const int *__restrict__ samples, // [batch_size]
    const __int128
        *__restrict__ intermediate_accum, // [batch_size, num_intermediates]
    __int128 *__restrict__ mol_energies   // [batch_size, num_energies]
...
template <typename RealType>
void __global__
k_compute_centroid_of_atoms(const int num_atoms,
  ...
  __syncthreads();
  if (threadIdx.x == 0) {
    centroid[0] = FIXED_TO_FLOAT<RealType>(fixed_centroid[0]) /
                  static_cast<RealType>(num_atoms);
    centroid[1] = FIXED_TO_FLOAT<RealType>(fixed_centroid[1]) /
                  static_cast<RealType>(num_atoms);
    centroid[2] = FIXED_TO_FLOAT<RealType>(fixed_centroid[2]) /
                  static_cast<RealType>(num_atoms);
  }
}

template void __global__ k_compute_centroid_of_atoms<float>(
    const int num_atoms, const int *__restrict__ atom_idxs,
    const float *__restrict__ coords, float *__restrict__ centroid);
template void __global__ k_compute_centroid_of_atoms<double>(
    const int num_atoms, const int *__restrict__ atom_idxs,
    const double *__restrict__ coords, double *__restrict__ centroid);

template <typename RealType>
void __global__ k_flag_mols_inner_outer(
    const int num_molecules, const int *__restrict__ atom_idxs,
    const int *__restrict__ mol_offsets, // [num_molecules + 1]
    const RealType
        *__restrict__ center, // [3] Center that determines inner vs outer
    const RealType
        square_radius, // squared radius from center that defines inner
    const RealType *__restrict__ coords, // [N, 3]
  ...
  while (mol_idx < num_molecules) {
    ...
    for (int atom_idx = start_idx; atom_idx < start_idx + num_atoms;
         atom_idx++) {
    ...
    RealType centroid_x = FIXED_TO_FLOAT<RealType>(centroid_accum_x) /
                          static_cast<RealType>(num_atoms);
    RealType centroid_y = FIXED_TO_FLOAT<RealType>(centroid_accum_y) /
                          static_cast<RealType>(num_atoms);
    RealType centroid_z = FIXED_TO_FLOAT<RealType>(centroid_accum_z) /
                          static_cast<RealType>(num_atoms);
    ...
    RealType dist = (centroid_x * centroid_x) + (centroid_y * centroid_y) +
...
void __global__ k_decide_targeted_moves(
    const int total_proposals, const int batch_size, const int num_target_mols,
    const int *__restrict__ noise_offset, // [1]
    const RealType *__restrict__ rand,    // [batch_size]
    const int *__restrict__ inner_count,  // [1]
    const RealType
        *__restrict__ translations, // [batch_size, 2, 3] first translation is
                                    // inside, second is outer
  ...
  while (idx < batch_size && rand_offset + idx < total_proposals) {
    ...
    // The inner translation is the first three values, the outer translations
    // is the next three.
    ...
    // The target weights will be the count of the target region + 1, the new
    // mol being inserted
    target_weights_counts[idx] =
        flag == 1 ? count_inside + 1 : count_outside + 1;
...
template void __global__ k_decide_targeted_moves<float>(
    const int total_proposals, const int batch_size, const int num_target_mols,
    const int *__restrict__ noise_offset, const float *__restrict__ rand,
    const int *__restrict__ inner_count, const float *__restrict__ translations,
...
template void __global__ k_decide_targeted_moves<double>(
    const int total_proposals, const int batch_size, const int num_target_mols,
    const int *__restrict__ noise_offset, const double *__restrict__ rand,
...
// k_separate_weights_for_targeted takes the flag and the mol indices and writes
// them out to a new buffer the weights associated with the source molecules.
template <typename RealType>
void __global__ k_separate_weights_for_targeted(
  ...
  const int weights_start =
      weight_offsets[idx_in_batch]; // Where to start adding the weights
  ...
  while (idx < count) {
    output_weights[weights_start + idx] =
        weights[partitioned_indices[idx + offset]];
...
void __global__ k_setup_destination_weights_for_targeted(
  ...
  // Don't set weights beyond the total number of proposals, state of buffers
  // can be invalid
  if (idx_in_batch >= batch_size ||
      idx_in_batch + num_prev_proposals >= total_proposals) {
    return;
  }
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  const int count = target_inner == 1 ? local_inner_count
                                      : num_target_mols - local_inner_count;
  const int offset = target_inner == 1 ? 0 : local_inner_count;
  // Handle the sampled molecule being moved from one region to another by
  // appending the sampled mol's weight to the target region's weight.
  if (idx == 0) {
    int sample_idx = samples[idx_in_batch];
    output_weights[weights_start + count + idx] =
        weights[idx_in_batch * num_target_mols + sample_idx];
  }
  while (idx < count) {
    output_weights[weights_start + idx] =
        weights[idx_in_batch * num_target_mols +
                partitioned_indices[idx + offset]];
...
void __global__ k_adjust_sample_idxs(
    ...
    const int *__restrict__ targeting_inner_volume, // [batch_size]
    const int *__restrict__ inner_count,            // [1]
    const int *__restrict__ partitioned_indices, // [num_mols] Total number of
                                                 // molecules being sampled
  ...
  // Only evaluate up to idx + num_prev_proposals < total proposals. The samples
  // beyond this point contain stale data due to k_decide_targeted_moves not
  // constructing them. All downstream kernels will use stale samples, but no
  // proposals past the total number proposals will be evaluated in
  // k_accept_first_valid_move_targeted
...
// k_accept_first_valid_move selects the first sample within a batch of samples.
// Use AtomicMin to find the lowest index sample that was selected, then store
// that sample in accepted sample field. If no sample is accepted store N in the
// accepted sample, indicating no valid sample. The accepted sample can also be
// used as the offset for shifting the noise for backtracking
template <typename RealType>
void __global__ k_accept_first_valid_move(
    const int total_proposals, const int num_target_mols, const int batch_size,
    const int *__restrict__ noise_offset,        // [1]
    const int *__restrict__ samples,             // [batch_size]
  ...
  __syncthreads();
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  const RealType before_log_prob = convert_nan_to_inf<RealType>(
      compute_logsumexp_final<RealType>(before_max[0], before_log_sum[0]));
  const int rand_offset = noise_offset[0];
  while (idx < batch_size) {
    ...
    const RealType after_log_prob = convert_nan_to_inf<RealType>(
        compute_logsumexp_final<RealType>(after_max[idx], after_log_sum[idx]));

    const RealType log_acceptance_prob =
        min(before_log_prob - after_log_prob, static_cast<RealType>(0.0));
    const bool accepted = rand[rand_offset + idx] < exp(log_acceptance_prob);
    if (accepted) {
      atomicMin(&selected_idx, idx);
      // Idx is increasing so the first accepted is the min value the thread can
      // accept making it safe to break early.
      break;
    }
...
void __global__ k_accept_first_valid_move_targeted(
  ...
  while (idx < batch_size) {
    ...
    const RealType raw_log_acceptance =
        compute_raw_log_probability_targeted<RealType>(
            targeting_inner_volume[idx], inner_volume, outer_vol,
            local_inner_count, num_target_mols, before_max + idx,
            before_log_sum + idx, after_max + idx, after_log_sum + idx);

    const RealType log_acceptance_prob =
        min(raw_log_acceptance, static_cast<RealType>(0.0));
    const bool accepted = rand[rand_offset + idx] < exp(log_acceptance_prob);
    if (accepted) {
      atomicMin(&selected_idx, idx);
      // Idx is increasing so the first accepted is the min value the thread can
...
template void __global__ k_store_exchange_move<float>(
    ...
    const int *__restrict__ mol_offsets,           // [num_mols + 1]
    const int *__restrict__ segment_offsets,       // [batch_size + 1]
    const float *__restrict__ moved_coords,        // [num_atoms_in_each_mol, 3]
    float *__restrict__ dest_coords,               // [N, 3]
    __int128 *__restrict__ before_energies,        // [num_target_mols]
    __int128 *__restrict__ after_energies, // [batch_size, num_target_mols]
    int *__restrict__ rand_offset,         // [1]
    int *__restrict__ inner_flags,         // [num_target_mols] or nullptr
