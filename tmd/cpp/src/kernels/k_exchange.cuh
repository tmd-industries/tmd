...
// Allows us to go from a clashy state to a non-clashy state. And no nan
// poisoning When we are considering exchange we want to treat Nan probabilities
// as inf Allows us to go from a clashy state to a non-clashy state. And no nan
// poisoning
template <typename RealType>
RealType __host__ __device__ __forceinline__
convert_nan_to_inf(const RealType input) {
  return isnan(input) ? static_cast<RealType>(INFINITY) : input;
}
...
template <typename RealType>
RealType __host__ __device__ __forceinline__
compute_log_proposal_probabilities_given_counts(const int src_count,
  ...
  // Invalid case
  assert(0);
  return 0.0; // Here to ensure the code compiles, assertion will trigger
              // failure
}
...
template <typename RealType>
RealType __host__ __device__ __forceinline__
compute_raw_log_probability_targeted(
    const int targeting_inner_volume, // 1 or 0
    const RealType inner_volume, const RealType outer_vol,
    const int inner_count, const int num_target_mols,
  ...
  const RealType log_vol_prob = targeting_inner_volume == 1
                                    ? log(inner_volume) - log(outer_vol)
                                    : log(outer_vol) - log(inner_volume);

  // Account for the proposal probability, in the case of there being 0 or 1 mol
  // in a volume the probability will not be symmetric.
  int src_count =
      targeting_inner_volume == 1 ? num_target_mols - inner_count : inner_count;
  int dest_count =
      targeting_inner_volume == 1 ? inner_count : num_target_mols - inner_count;
  const RealType log_fwd_prob =
      compute_log_proposal_probabilities_given_counts<RealType>(src_count,
                                                                dest_count);
  const RealType log_rev_prob =
      compute_log_proposal_probabilities_given_counts<RealType>(src_count - 1,
                                                                dest_count + 1);

  RealType before_log_prob = convert_nan_to_inf<RealType>(
      compute_logsumexp_final<RealType>(before_max[0], before_log_sum[0]));
  RealType after_log_prob = convert_nan_to_inf<RealType>(
      compute_logsumexp_final<RealType>(after_max[0], after_log_sum[0]));

  return before_log_prob - after_log_prob + log_vol_prob +
         (log_rev_prob - log_fwd_prob);
}
...
template <typename RealType>
void __global__ k_compute_box_volume(const RealType *__restrict__ box,
                                     RealType *__restrict__ output_volume);

// k_adjust_energies takes a set of molecules and either subtracts
// (Negated=true) or adds (Negated=false) the sum of the per atom energies for
// the molecules from some initial energies. This is used to do the
// transposition trick where we subtract off the weight contribution of the
// moved atom followed by adding back in the weight of the sampled mol in the
// new position. Does NOT special case the weight of the sampled mol and instead
// use `k_set_sampled_energy`.
...
// k_adjust_sample_idxs is to to handle that in the TIBD exchange move the
// initial samples are selected from either the inner or outer volume. So the
// indices will go from 0 -> len(inner) or len(outer) depending on the region
// being sampled. All of the downstream code expect the sample idxs to be from 0
// -> num_mols This kernel simple adjusts the sample idxs to match that
// expectation.
