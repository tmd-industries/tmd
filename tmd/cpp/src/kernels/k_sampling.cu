...
void __global__
k_setup_gumbel_max_trick(const int N,
...
template void __global__ k_setup_gumbel_max_trick<float>(
    const int N, const float *__restrict__ log_weights,
...
template void __global__ k_setup_gumbel_max_trick<double>(
    const int N, const double *__restrict__ log_weights,
...
void __global__ k_setup_gumbel_max_trick_with_offset(
  ...
  // In the case of the offset the values per segment need to match up to
  // compute gumbel offset correctly.
...
template void __global__ k_copy_kv_key<float>(
    const int N, const cub::KeyValuePair<int, float> *__restrict__ kv_pairs,
    int *__restrict__ out);

template void __global__ k_copy_kv_key<double>(
    const int N, const cub::KeyValuePair<int, double> *__restrict__ kv_pairs,
    int *__restrict__ out);

/* k_setup_gumbel_max_trick_targeted_insertion is a specialized version of
 * k_setup_gumbel_max_trick_with_offset to handle the fact that we generate
 * num_noise_per_segment values of noise per segment to ensure that we get
 * bitwise deterministic results. If k_setup_gumbel_max_trick_with_offset were
 * to be used, the data would be re-used but you wouldn't be able to get the
 * same results depending on the number of proposals per move.
 *
 ...
 * outer_count = 1
 *
 * If the proposals_per_move were 1, and had two iterations the results would
 * look like: Iteration 1 Move targeting inner (evaluating the single outer
 * molecule) noise = [a, b, c, d] ^ Only use the first value weights = [1]
 *     Sampler -> 0 // First idx
 * Iteration 2
 ...
 *
 * If the proposals_per_move were 2, and had one iteration the results would
 * look like: Iteration 1 First targeting inner (evaluating the single outer
 * molecule) then outer (three mols) noise = [a, b, c, d] ^ Used b noise for 2
 * rather than e in the case of a single proposal per move weights = [[1], [2,
 * 3, 4]] Sampler -> [0, 0] // First idxs of both segments selected, rather than
 * first and second idx.
 *
 * This kernel forces such that each set of weights always uses the full
 * num_noise_per_segment, avoiding the difference between proposals_per_move.
 */
template <typename RealType>
void __global__ k_setup_gumbel_max_trick_targeted_insertion(
    const int num_segments, const int num_noise_per_segment,
  ...
  const int gumbel_offset =
      rand_offset * num_noise_per_segment + segment_idx * num_noise_per_segment;
...
template void __global__ k_setup_gumbel_max_trick_targeted_insertion<float>(
    const int num_segments, const int num_noise_per_segment,
    const int max_offset, const int *__restrict__ noise_offset,
    const int *__restrict__ segment_offsets, // [blockDim.y]
    const float *__restrict__ log_weights,
    const float *__restrict__ gumbel_noise,
    float *__restrict__ prepared_gumbel);
