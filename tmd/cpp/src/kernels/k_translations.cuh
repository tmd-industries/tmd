...
// translations inside of a sphere and outside of a sphere depending on if the
// idx is odd or even. If the idx is even the translation will be inside the
// sphere and odd will be outside the sphere
template <typename RealType>
void __global__ k_generate_translations_inside_and_outside_sphere(
    const int num_translations, const RealType *__restrict__ box,
  ...
  const int total_translations = num_translations * 2;

  // Generate twice the number of translations, each translation will produce an
  // inner and outer translation
  while (idx < total_translations) {
  ...
  __syncthreads();
  // Cycle the index that the states are written to, this ensures that calling
  // TIBD with 1 step or 20 steps produces the same sequence of translations.
  int new_idx = threadIdx.x - ((total_translations) % block_size);
  new_idx = new_idx >= 0 ? new_idx : block_size + new_idx;
  // Write back out the states to ensure that the next time the kernel is called
  // the random states are different
