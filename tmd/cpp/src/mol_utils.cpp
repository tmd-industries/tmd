...
void verify_mols_contiguous(const std::vector<std::vector<int>> &group_idxs) {
  ...
  for (unsigned int i = 0; i < group_idxs.size(); i++) {
    ...
    if (atoms[0] != last_water_end + 1) {
      throw std::runtime_error("Molecules are not contiguous: mol " +
                               std::to_string(i));
    }
    ...
    for (int j = 1; j < num_atoms; j++) {
      if (last_atom + 1 != atoms[j]) {
        throw std::runtime_error("Molecule " + std::to_string(i) +
                                 "is not sequential in atom indices");
      }
      last_atom = atoms[j];
    }
...
prepare_group_idxs_for_gpu(const std::vector<std::vector<int>> &group_idxs) {
  ...
  for (int i = 0; i < num_mols; i++) {
    std::vector<int> atoms = group_idxs[i];
    // ASSUMES MOLS ARE MADE UP CONTIGIOUS INDICES IE mols[0] = [0, 1, 2],
    // mols[1] = [3, 2], etc IF mols[0] = [5, 7], mols[1] = [0, 8], THIS WON'T
    // WORK. If this is not the case need a complete mapping of atom_idx to
    // mol_idx Sort the atom indices from smallest to largest so that you can
    // know the range of indices in the molecule
    std::sort(atoms.begin(), atoms.end());
