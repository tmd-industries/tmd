...
const static int THREADS_PER_BLOCK =
    64; // 64 threads seem to produce the best outcome on Cuda Arch 8.6
...
NonbondedMolEnergyPotential<RealType>::NonbondedMolEnergyPotential(
  ...
  std::array<std::vector<int>, 3> target_flattened_groups =
      prepare_group_idxs_for_gpu(target_mols);
  ...
  // Need per atom energies per block to account for accumulating between
  // different x blocks that have the same y block index
...
void NonbondedMolEnergyPotential<RealType>::mol_energies_device(
    const int N, const int target_mols,
  ...
  gpuErrchk(cudaMemsetAsync(d_atom_energy_buffer_.data, 0,
                            d_atom_energy_buffer_.size(), stream));

  // Compute the per atom energies of all of the molecules of interest
  k_compute_nonbonded_target_atom_energies<RealType, tpb>
  ...
  k_accumulate_atom_energies_to_per_mol_energies<RealType, BLOCK_SIZE>
      <<<target_mols, BLOCK_SIZE, 0, stream>>>(
          target_mols, d_target_mol_offsets_.data, d_atom_energy_buffer_.data,
          d_output_energies);
  gpuErrchk(cudaPeekAtLastError());
}
...
std::vector<__int128> NonbondedMolEnergyPotential<RealType>::mol_energies_host(
    const int N, const int P, const RealType *h_coords,
    const RealType *h_params, const RealType *h_box) {
  ...
  this->mol_energies_device(N, num_target_mols_, d_coords.data, d_params.data,
                            d_box.data, d_output.data, stream);
