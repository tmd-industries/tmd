...
/*
Optimally align x2 onto x1. In particular, x2 is shifted so that its centroid is
placed at the same position as the of x1's centroid. x2 is also rotated so that
the RMSD is minimized.
*/
void rmsd_align_cpu(const int N, const double *x1_raw, const double *x2_raw,
  ...
  Eigen::MatrixXd x2_aligned =
      x2_rot.rowwise() - (translation.transpose() - x2_centroid.transpose());
