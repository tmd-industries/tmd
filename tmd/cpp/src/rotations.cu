...
void rotate_coordinates_host(const int N, const int n_rotations,
  ...
  DeviceBuffer<RealType> d_out(n_rotations * N * 3);
  ...
  k_rotate_coordinates<RealType><<<dimGrid, tpb, 0, stream>>>(
      N, n_rotations, d_coords.data, d_quats.data, d_out.data);
...
void rotate_coordinates_and_translate_mol_host(
  ...
  DeviceBuffer<RealType> d_out(d_coords.length * batch_size);
  ...
  k_rotate_and_translate_mols<RealType, true>
      <<<ceil_divide(batch_size, tpb), tpb, 0, stream>>>(
          batch_size, batch_size, d_offset.data, d_coords.data, d_box.data,
          d_samples.data, d_mol_offsets.data, d_quaternion.data,
          d_translation.data, d_out.data);
  gpuErrchk(cudaStreamSynchronize(stream));
  d_out.copy_to(output);
}

template void rotate_coordinates_host<float>(const int, const int,
                                             const float *, const float *,
                                             float *);
template void rotate_coordinates_host<double>(const int, const int,
