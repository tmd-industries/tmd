...
def draw_mol(mol: Chem.Mol, highlightAtoms, highlightColors):
    """from YTZ, Feb 1, 2021"""
...
def draw_mol_idx(mol: Chem.Mol, highlight: Optional[list[int]] = None, scale_factor=None):
    """
...
def recenter_mol(mol: Chem.Mol) -> Chem.Mol:
...
def rotate_mol(mol: Chem.Mol, rotation_matrix) -> Chem.Mol:
...
def get_romol_conf(mol: Chem.Mol, conf_id: int = 0) -> NDArray:
    """Coordinates of the specified conformer, in nanometers"""
    conformer = mol.GetConformer(conf_id)
    guest_conf = np.array(conformer.GetPositions(), dtype=np.float32)
    return guest_conf / 10.0  # from angstroms to nm


def set_romol_conf(mol: Chem.Mol, new_coords: NDArray, conf_id: int = 0):
...
def get_mol_name(mol: Chem.Mol) -> str:
    """Return the title for the given mol"""
    return mol.GetProp("_Name")


def set_mol_name(mol: Chem.Mol, name: str):
    """Set an RDKit mol's name"""
    mol.SetProp("_Name", name)


def is_macrocycle(mol: Chem.Mol) -> bool:
    """Determines if a molecule is a macrocycle. Considered a macrocycle if the largest
    ring is greater than 10 atoms.

    Parameters
    ----------
    mol: rdkit mol
        Molecule to evaluate for being a macrocycle
    ...
    return largest_ring_size > 10


def generate_conformations(
    mol, n_confs: int = 800, rms_threshold: float = 1.0, clear_existing_confs: bool = True, seed: int = 2025
):
    """Generate conformations for an RDKit Molecule. Correctly handles macrocycles.

    Parameters
    ----------
    mol: rdkit mol
        Input mol to add conformers to

    n_confs: int
        Maximum number of conformers to generate.
        Defaults to 800 conformers to conform to oe_generate_conformations

    rms_threshold: float
        RMS threshold to prune conformers with. Units are in angstroms.

    clear_existing_confs: bool
        Whether or not to remove existing conformers.

    seed: int
        Seed to generate conformers with
    """
    try:
    ...
    params.clearConfs = clear_existing_confs
    params.randomSeed = seed

    confs = len(AllChem.EmbedMultipleConfs(mol, n_confs, params))
    if confs == 0:
        if original_conf is None:
            raise RuntimeError("Failed to generate any conformers and no original conformer available")
