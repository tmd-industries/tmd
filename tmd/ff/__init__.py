...
from tmd.constants import DEFAULT_FF, DEFAULT_PROTEIN_FF, DEFAULT_WATER_FF
from tmd.ff.handlers import bonded, nonbonded
from tmd.ff.handlers.deserialize import deserialize_handlers
from tmd.ff.handlers.serialize import serialize_handlers
...
class Forcefield:
    ...
    q_handle: Optional[
        Union[
            ...
            nonbonded.PrecomputedCCCHandler,
            nonbonded.NNHandler,
        ]
    ]
    q_handle_intra: Optional[
        Union[
            ...
            nonbonded.PrecomputedChargeHandler,
            nonbonded.PrecomputedCCCIntraHandler,
            nonbonded.NNHandler,
        ]
    ]
    ...
    def load_from_file(cls, path_or_str: Union[str, Path]) -> "Forcefield":
        ...
        path_or_str: string or pathlib.Path
            Either the filename of a built in ff (smirnoff_2_0_0_sc.py) or a path to a new forcefield file
        ...
        # Look for builtin ff
        with resources.as_file(resources.files("tmd.ff.params") / path.name) as rpath:
            if rpath.exists():
                if path.is_file():
                    warn(
    ...
    def load_precomputed_default(cls) -> "Forcefield":
        ...
        return cls.load_from_file("smirnoff_2_0_0_precomputed.py")
    ...
    def from_handlers(
        ...
        for handle in ff_handlers:
            if isinstance(
                ...
                (
                    ...
                    nonbonded.PrecomputedCCCSolventHandler,
                ),
            ):
        ...
        if q_handle_intra is None and q_handle is not None:
            ...
            elif isinstance(q_handle, nonbonded.PrecomputedCCCHandler):
                q_handle_intra = nonbonded.PrecomputedCCCIntraHandler(q_handle.smirks, q_handle.params, q_handle.props)
