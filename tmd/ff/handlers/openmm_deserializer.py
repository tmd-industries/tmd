...
def deserialize_system(system: mm.System, cutoff: float) -> tuple[list[potentials.BoundPotential], NDArray]:
    ...
    if len(nb_forces) > 0:
        ...
        nb_params, exclusion_idxs, _, scale_factors = deserialize_nonbonded_force(nb_forces[0], N)

        nonbonded = potentials.Nonbonded(N, exclusion_idxs, scale_factors, cutoff).bind(nb_params)
