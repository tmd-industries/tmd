...
def sequential_monte_carlo(
    ...
    store_intermediate_traj: bool = True,
    max_num_lambdas: int = 1000,
    initial_lamb: float = 0.0,
    target_lamb: float = 1.0,
) -> ResultDict:
    ...
    resample: function
        perform resampling given an array of log weights
    find_next_lambda: function
        function that defines the lambda schedule and computes the incremental log weights
    ...
    target_lamb: float
        Value of lambda to terminate at
    ...
    current_lamb: Lambda = initial_lamb
    next_lamb: Lambda = target_lamb
    current_iteration: Iteration = 0

    for _ in range(max_num_lambdas):
        next_lamb, incremental_log_weights = find_next_lambda(
            sample_traj[-1], current_lamb, current_iteration, norm_log_weights
        )

        # Stop when next_lamb == target_lamb
        #   See
        ...
        # propagate
        samples = propagate(resampled, next_lamb)

        # log
        accumulate_results(samples, indices, log_weights, incremental_log_weights, next_lamb)
    ...
    lambdas_traj.append(target_lamb)
...
def fixed_find_next_lambda(
    ...
    * [Dai, Heng, Jacob, Whiteley, 2020] An invitation to sequential Monte Carlo samplers
    ...
    """
    deltas = np.diff(lambdas)
    # Should be monotonically increasing or decreasing in lambda
    assert np.all(np.sign(deltas) == np.sign(deltas[0])), (
        "lambda schedule must be monotonically increasing or decreasing"
    )
...
def adaptive_find_next_lambda(
    ...
    """
    Implementation of Adaptive Sequential Monte Carlo (SMC).
    This will adaptively interpolate from current_lambda to final_lambda.
    ...
    max_iterations:
        ...
        `SMCMaxIterError` exception.
    final_lambda: float
        Lambda value to move towards. Must be in the interval [0.0, 1.0]
    Returns
    -------
    2-tuple: target lambda, incremental log weights
