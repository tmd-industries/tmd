...
def call_unbound_impl(
    impl: custom_ops.Potential_f32 | custom_ops.Potential_f64, conf: NDArray, params: NDArray, box: NDArray
) -> float:
...
@partial(jax.custom_jvp, nondiff_argnums=(0,))
def call_bound_impl(
    impl: custom_ops.BoundPotential_f32 | custom_ops.BoundPotential_f64, conf: NDArray, box: NDArray
) -> float:
...
def _(impl: custom_ops.Potential_f32 | custom_ops.Potential_f64, primals, tangents) -> tuple[Any, Any]:
    x, p, box = primals
    dx, dp, dbox = tangents
    ...
    # (YTZ: HACK, python interprets float32 as 'float' types, which are 64bit in python)
...
def _(impl: custom_ops.BoundPotential_f32 | custom_ops.BoundPotential_f64, primals, tangents) -> tuple[Any, Any]:
    x, box = primals
    dx, dbox = tangents
