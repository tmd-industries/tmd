...
def pairs_from_interaction_groups(group_a_indices: Array, group_b_indices: Array) -> Array:
    ...
    if len(group_b_indices) >= len(group_a_indices):
        is_overlapping = bool(np.all(group_a_indices == group_b_indices[: len(group_a_indices)]))
    else:
        is_overlapping = False

    if is_overlapping:
        pair_list = []
        for idx, val_i in enumerate(group_a_indices):
            for jdx, val_j in enumerate(group_b_indices):
                if jdx > idx:
                    pair_list.append((val_i, val_j))
