...
def combining_rule_epsilon(eps_i, eps_j):
    ...
    (point 2 of https://github.com/badisa/tmd#forcefield-gotchas)
    """
    return eps_i * eps_j
...
def reaction_field_electrostatics(dij, qij, cutoff):
    """Reaction Field from eq 11 of:
    "Cisneros GA, et al 2015
...
def validate_interaction_group_idxs(n_atoms, a_idxs, b_idxs):
    """assert A and B are disjoint or that A is a prefix of B, that neither contain no elements outside range(0, n_atoms), and there are no repeats"""
    ...
    if len(b_idxs) >= len(a_idxs):
        is_prefix = np.all(a_idxs == b_idxs[: len(a_idxs)])
    else:
        is_prefix = False
    assert is_disjoint ^ is_prefix
    AB = A.union(B)
